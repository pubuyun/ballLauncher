...
def _start_camera():
    ...
    with _cam_lock:
        ...
        if not cap.isOpened():
            ...
            print(
                f"[{_ts()}] [CAM] Webcam opened index={_cam_index} size={_cam_width}x{_cam_height} fps~{_cam_fps}"
            )
        _cam = cap
